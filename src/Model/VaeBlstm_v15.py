...
from Model.Model1 import oneLayerBidirectionalLstmEncoder, vaeSampler,\
    ...
    twoLayerFcEncoder, twoLayerFcDecoder
...
class VaeBlstm_v15(BaseNetwork):
    '''
    classdocs
    '''
    ...
    def __init__(self, sampleDim, segLen_max, mConfig):
        ...
        super(VaeBlstm_v15, self).__init__(sampleDim, mConfig)
    ...
    def buildNet(self):
        ...
        self.sample_forEncoder = tf.reshape(self.placeHolder_samples, [-1, self.sampleDim], 'vae_encoder_reshape')
        
        self.vae_mean, self.vae_SE_ln = twoLayerFcEncoder(self.sample_forEncoder, self.hiddenDim_vae_encoder, self.hiddenDim_vae_z)
        
        
        self.Z = vaeSampler(self.vae_mean, self.vae_SE_ln, tf.reshape(self.placeHolder_noises, [-1, self.hiddenDim_vae_z]))
        self.reconstruction_vae = tf.reshape(twoLayerFcDecoder(self.Z,  self.hiddenDim_vae_decoder, self.reconstructionTargetDim), [-1, self.segLen_max, self.sampleDim], 'vae_reconstruction')

        self.Z_res = tf.reshape(self.Z, [-1, self.segLen_max, self.hiddenDim_vae_z], 'vae_z_rs')
        ...
        self.loss_vae, self.KLD, _ = vaeLoss_forLstm1(self.reconstruction_vae, self.placeHolder_targets_reconstruction, \
                                                      tf.reshape(self.vae_mean_squared, [-1, self.segLen_max, self.hiddenDim_vae_z]), \
                                                      tf.reshape(self.vae_SE, [-1, self.segLen_max, self.hiddenDim_vae_z]), \
                                                      tf.reshape(self.vae_SE_ln, [-1, self.segLen_max, self.hiddenDim_vae_z]))
        ...
        if self.mConfig.finetune is False:
            ...
            self.meanLoss = tf.divide(tf.reduce_sum(self.maskedLoss_vae), tf.reduce_sum(self.mask))
            
        else:
            self.prediction = blstmSequencePredictionNetwork(self.Z_res, self.placeHolder_seqLens, self.segLen_max, \
    ...
    def testOneEpoch(self):
        ...
        if self.mConfig.finetune is True:
            ...
            for batchIte in range(self.batchLoader_test.getBatchNumPerEpoch()):
                ...
                currentFeed = {self.placeHolder_samples: sampleBatch_test, self.placeHolder_targets_reconstruction: sampleBatch_test, \
                               self.placeHolder_noises: noise, self.placeHolder_labels: frameLabelBatch_test[:,0], \
                               self.placeHolder_seqLens: seqlenBatch_test, self.placeHoder_learningRate: self.currentLearningRate}
